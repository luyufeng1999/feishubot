...
CARD_URI = "/open-apis/message/v4/send"

GET_USER_INFO_URI = "/open-apis/contact/v3/users"
...
class UserApiClient(object):
    ...
    def get_user_info_with_user_id(self, user_id):
        return self.get_user_info("user_id", user_id)

    def get_user_info(self, user_id_type, user_id):
        self._authorize_tenant_access_token()
        url = "{}{}/{}?user_id_type={}".format(
        self._lark_host, GET_USER_INFO_URI, user_id, user_id_type)
        headers = {
            "Authorization": "Bearer " + self.tenant_access_token,
        }
        resp = requests.get(url=url, headers=headers)
        return json.loads(resp.text).get("data").get("user")
...
class MessageApiClient(object):
    ...
    def send_card_with_open_id(self, open_id, content):
        self.send_card("open_id", open_id, content)


    def send_card(self, receive_id_type, receive_id, content):
        self._authorize_tenant_access_token()
        url = "{}{}".format(
            self._lark_host, CARD_URI
        )
