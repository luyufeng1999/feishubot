...
class MessageCardCallbackEvent():
    def __init__(self, dict_data, token, encrypt_key):
        data = dict_2_obj(dict_data)
        self.open_id = data.open_id
        self.user_id = data.user_id
        self.open_message_id = data.open_message_id
        self.open_chat_id = data.open_chat_id
        self.tenant_key = data.tenant_key
        self.token = token
        self.action = data.action

    @staticmethod
    def event_type():
        return "message_card.callback"
...
class EventManager(object):
    ...
    event_type_map = dict()
    msg_card_callback_map = dict()
    _event_list = [MessageReceiveEvent, UrlVerificationEvent, MessageCardCallbackEvent]
    ...
    def get_handler_with_event(token, encrypt_key):
        ...
        if not ("token" in dict_data.keys()): 
        ...
        event = dict_data.get("header")
        # get event_type
        if event is None:
            event_type = "message_card.callback"
        else:
